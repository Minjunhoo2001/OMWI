...
coef_file <- get_arg("--coef", "model/omhi_species_coefficients.tsv")
universe_file <- get_arg("--universe", "model/omhi_species_universe.tsv")
output_file <- get_arg("--output", "omhi_scores.tsv")
pseudocount <- as.numeric(get_arg("--pseudocount", "1e-6"))

if (is.null(input_file)) {
  stop("Usage: Rscript calc_omhi.R --input species_abundance.tsv --coef model/omhi_species_coefficients.tsv --universe model/omhi_species_universe.tsv --output omhi_scores.tsv")
}

cat("[OMHI] Reading input species table:", input_file, "\n")
...
cat("[OMHI] Reading universe:", universe_file, "\n")
...
cat("[OMHI] Reading coefficients:", coef_file, "\n")
...
if (median(sample_sums, na.rm = TRUE) > 2) {
  cat("[OMHI] Input appears to be percentage scale. Dividing by 100.\n")
  X2 <- X2 / 100
}

# Align to fixed OMHI universe
X_uni <- matrix(
  0,
...
cat("[OMHI] Matched universe taxa:", length(matched_taxa), "/", length(universe_taxa), "\n")
cat("[OMHI] Missing universe taxa were assigned zero abundance.\n")
...
if (length(missing_coef_taxa) > 0) {
  stop("Some coefficient taxa are not in the OMHI universe: ", paste(missing_coef_taxa, collapse = ", "))
}
...
out <- data.frame(
  ...
  OMHI = score,
  predicted_state = ifelse(score > 0, "health_associated", "disease_associated"),
